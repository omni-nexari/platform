...
export async function init(cfg: SyncConfig): Promise<void> {
  ...
  logger.info(`[Sync] init deviceId=${cfg.deviceId} group=${cfg.groupId} role=${_role} pinned=${cfg.pinnedLeaderId ?? 'none'}`);
  ...
  if (resolvedRole === 'leader') {
    ...
    // LOAD_URL again unless we ask. Request resync after 8 s if no LOAD_URL.
...
async function _runLeader(): Promise<void> {
  ...
  // and call _resyncLeader again → destroyEngine stopped the leader in a loop.
  try {
    await _cfg.prepareEngine(url);
  } catch (e: unknown) {
    logger.error(`[Sync] leader prepare failed: ${(e as Error)?.message} — will retry on next peer scan`);
    return;
  }
  if (_stopped) return;
...
function _scheduleFollowerResync(delayMs = 4000): void {
  ...
  _followerResyncTimer = setTimeout(() => {
    ...
    if (_peers.length === 0) { _scheduleFollowerResync(3000); return; }
