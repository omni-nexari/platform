...
export type PlatformOtaProgressKind =
...
export interface PlatformAdapter {
  ...
  reboot(): Promise<{ supported: boolean; error?: string }>;
  /** Soft power off / standby. */
  powerOff(): Promise<{ supported: boolean; error?: string }>;
  /** Wake from standby. No-op on devices that auto-wake. */
  powerOn(): Promise<{ supported: boolean; error?: string }>;
  /** Lock the screen / blank the display without quitting the app. */
  sleep(): Promise<{ supported: boolean; error?: string }>;
  /** Current power state as the host sees it. */
  getPowerState(): Promise<PlatformPowerState>;

  // ── Audio / display ───────────────────────────────────────────────────────
  /** 0..100 — media stream on Android. */
  setVolume(pct: number): Promise<{ supported: boolean }>;
  getVolume(): Promise<number>;
  setMute(muted: boolean): Promise<{ supported: boolean }>;
  /** 0..100. */
  setBrightness(pct: number): Promise<{ supported: boolean }>;
  getBrightness(): Promise<number>;

  // ── Screenshot ────────────────────────────────────────────────────────────
  /** Returns JPEG bytes of the current player surface. */
  screenshot(): Promise<{ jpegBase64: string; width: number; height: number } | null>;
  ...
  lockKiosk(): Promise<{ supported: boolean }>;
  unlockKiosk(): Promise<{ supported: boolean }>;
