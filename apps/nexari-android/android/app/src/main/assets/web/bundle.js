...
function renderCalendar(container, content, api, deviceId, ws, registerPushHandler) {
  ...
  const renderMeetingRoomLobby = (events) => {
    var _a3;
    const now = getNow();
    const startOfDay = /* @__PURE__ */ new Date();
    startOfDay.setHours(0, 0, 0, 0);
    const endOfDay = new Date(startOfDay);
    endOfDay.setDate(endOfDay.getDate() + 1);
    const today = events.filter((e) => !e.allDay && new Date(e.end).getTime() > startOfDay.getTime() && new Date(e.start).getTime() < endOfDay.getTime()).sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime());
    const currentEv = today.find((e) => new Date(e.start).getTime() <= Date.now() && new Date(e.end).getTime() > Date.now());
    const nextEv = (_a3 = today.find((e) => new Date(e.start).getTime() > Date.now())) != null ? _a3 : null;
    const highlightEv = currentEv != null ? currentEv : nextEv;
    const roomName = (roomMeta == null ? void 0 : roomMeta.name) || content.name || "Meeting Room";
    const locationStr = (roomMeta == null ? void 0 : roomMeta.location) || "";
    const lobbyBookingUrl = (roomMeta == null ? void 0 : roomMeta.bookingUrl) || "";
    const showAtt = !!theme.showAttendeeCount;
    const lobbyMain = "#1446c0";
    const lobbyPanel = "#0d2f7e";
    const lobbyCyan = "#22d3ee";
    const fmtRange = (e) => `${fmtClockTime(toLocal(e.start))} \u2013 ${fmtClockTime(toLocal(e.end))}`;
    const listedEvs = today.filter((e) => new Date(e.end).getTime() > Date.now()).slice(0, 5);
    const evItemsHtml = listedEvs.length === 0 ? `<div style="color:rgba(255,255,255,0.45);font-size:15px;margin-top:8px;">No upcoming events today</div>` : listedEvs.map((ev) => {
    ...
    const rightContent = highlightEv ? `
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;">
        ${tappable ? `<button data-lobby-enter style="padding:12px 40px;background:transparent;border:2px solid rgba(255,255,255,0.55);border-radius:6px;color:#fff;font-size:20px;font-weight:700;letter-spacing:2px;cursor:pointer;font-family:inherit;text-transform:uppercase;">ENTER</button>` : `<div></div>`}
        ${highlightCount !== null ? `<div style="background:rgba(255,255,255,0.12);border-radius:10px;padding:10px 18px;text-align:center;border:1px solid rgba(255,255,255,0.2);">
    ...
    if (tappable) {
      container.querySelectorAll("[data-lobby-enter],[data-lobby-book]").forEach((btn) => {
        btn.addEventListener("click", () => {
          try {
            window.open(lobbyBookingUrl, "_blank", "noopener");
          } catch (e) {
          }
        });
      });
  ...
  const renderEvents = (events) => {
    ...
    switch (view) {
      ...
      case "meeting_room":
        ...
        break;
      case "meeting_room_lobby":
        renderMeetingRoomLobby(events);
        break;
    }
  };
...
var DeviceMessageSchema = external_exports.discriminatedUnion("type", [
  ...
  external_exports.object({
    type: external_exports.literal("firmware_update_complete"),
    payload: external_exports.object({
      ok: external_exports.boolean(),
      swVersion: external_exports.string().optional(),
...
<div style="text-align:center;max-width:680px;width:90%;padding:40px 0;position:relative;z-index:1;">
  ...
  <!-- Logo -->
  <img src="./nexari-logo.png" alt="${escapeHtml3(brandName)}"
  ...
  <div style="display:flex;flex-wrap:wrap;justify-content:center;gap:8px 10px;
              ...
              font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;">
    <span style="padding:7px 12px;border-radius:8px;border:1px solid rgba(255,255,255,0.08);border-left:3px solid rgba(58,123,255,0.55);background:linear-gradient(180deg,rgba(255,255,255,0.05) 0%,rgba(255,255,255,0.02) 100%);">Model: <strong style="color:#cbd5e1;">${escapeHtml3(info.modelName || info.modelCode || "\u2014")}</strong></span>
    <span style="padding:7px 12px;border-radius:8px;border:1px solid rgba(255,255,255,0.08);border-left:3px solid rgba(58,123,255,0.55);background:linear-gradient(180deg,rgba(255,255,255,0.05) 0%,rgba(255,255,255,0.02) 100%);">IP: <strong style="color:#cbd5e1;">${escapeHtml3(net.ipAddress || "Connecting\u2026")}</strong></span>
    <span style="padding:7px 12px;border-radius:8px;border:1px solid rgba(255,255,255,0.08);border-left:3px solid rgba(58,123,255,0.55);background:linear-gradient(180deg,rgba(255,255,255,0.05) 0%,rgba(255,255,255,0.02) 100%);">Serial: <strong style="color:#cbd5e1;">${escapeHtml3(info.serialNumber || "\u2014")}</strong></span>
    <span style="padding:7px 12px;border-radius:8px;border:1px solid rgba(255,255,255,0.08);border-left:3px solid rgba(58,123,255,0.55);background:linear-gradient(180deg,rgba(255,255,255,0.05) 0%,rgba(255,255,255,0.02) 100%);">Platform: <strong style="color:#cbd5e1;">${escapeHtml3(info.platform || "\u2014")}</strong></span>
  </div>

  <!-- URL hint -->
  <p style="font-size:14px;color:#555f75;margin:0 0 28px;">
    Dashboard: <span style="color:#4ff2d1;">${escapeHtml3(serverHost || "")}</span>
  </p>
