  ...
  async getSystemInfo(forceRefresh = false) {
    ...
    self._systemInfoInFlight = (async () => {
      ...
      const safeNetworkCall = (methodName, ...args) => {
        try {
          if (networkApi && typeof networkApi[methodName] === 'function') {
            return networkApi[methodName](...args);
          }
        } catch (err) {
          logger.debug(`webapis.network.${methodName} failed:`, (err && err.message) || err);
        }
        return null;
      };
      ...
      // Calculate uptime (time since player started)
      ...
      const systemInfo = {
        ...
        firmwareVersion: (build && build.buildVersion) || null,
        ...
        memoryTotal: (memory && memory.availableCapacity) ? memory.availableCapacity * 2 : null, // Estimate
        memoryFree: (memory && memory.availableCapacity) || null,
        memoryAvailable: (memory && memory.availableCapacity) || null,
  ...
  async send(deviceId) {
    try {
      ...
      const telemetryData = {
        ...
        // B2B display name (webapis.network.getTVName / getDeviceName)
        tvName: systemInfo.tvName || undefined,
      };
