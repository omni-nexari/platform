...
function buildCurrentClockPacket() {
  ...
  var cH12   = cH24 % 12 || 12;
  return buildPacket(CMD_CLOCK, [cDay, cH12, cMin, cMonth, cYear1, cYear2, cAmpm], 0x01);
}
  ...
  if (req.method === 'GET' && req.url === '/status-full') {
    ...
    function sfStep3(next) {
      sendMdcPacketWithResponse(buildPacket(CMD_DEVICE_NAME, []), function(err, buf) {
        if (!err && buf && buf.length >= 5 && String.fromCharCode(buf[4]) === 'A') {
          var dLen = buf[3];
          var dName = '';
  ...
  if (req.method === 'POST' && req.url === '/mdc-control') {
    ...
    req.on('end', function() {
      ...
      if (action === 'set_volume') {
        ...
        // MDC §2.1.67 SET: variable-length name bytes, max 15 chars, ID 0x01
        var devName = (parsed.name || '').slice(0, 15);
        var nameBytes = [];
        for (var ni = 0; ni < devName.length; ni++) {
          nameBytes.push(devName.charCodeAt(ni));
        }
        packet = buildPacket(CMD_DEVICE_NAME, nameBytes, 0x01);
        ...
        sendMdcPacketWithResponse(clockPkt, function(clockErr, clockBuf) {
          ...
          if (clockAck !== 'A') {
            ...
            res.writeHead(502, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ ok: false, error: 'set_clock NAK 0x' + nakCode, rawHex: bufferHex(clockBuf) }));
            return;
          }
          res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        sendMdcPacketWithResponse(buildPacket(CMD_CLOCK, []), function(err, buffer) {
          ...
          if (ack !== 'A') {
            res.writeHead(502, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ ok: false, error: 'MDC NAK for clock', rawHex: rawHex }));
            return;
          }
        ...
        sendMdcPacketWithResponse(buildPacket(timerCmd, []), function(err, buf) {
          ...
          if (ack !== 'A') {
            res.writeHead(502, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ ok: false, error: 'MDC NAK for on_timer slot ' + slot, rawHex: rawHex }));
            return;
          }
        ...
        sendMdcPacketWithResponse(buildPacket(CMD_AUTO_ID, [0x01], 0xFE), function(err, buf) {
          ...
          setTimeout(function() {
            scanMdcId([1,2,3,4,5,6,7,8,9], 800, function(scanErr, found) {
              res.writeHead(200); res.end(JSON.stringify({
                ok: !!found,
                stopped: true,
                ackReceived: ackOk,
        ...
        var newId = Math.max(1, Math.min(254, parseInt(parsed.id, 10) || 1));
        DEVICE_MDC_ID = newId;
        res.writeHead(200); res.end(JSON.stringify({ ok: true, displayId: DEVICE_MDC_ID }));
        return;
      } else if (action === 'mdc_id_scan') {
        // Scan MDC IDs 1–9 to find which one this device is set to.
        // Sends CMD_STATUS GET to each ID sequentially, 800 ms timeout per ID.
        // Automatically updates DEVICE_MDC_ID on success.
        scanMdcId([1,2,3,4,5,6,7,8,9], 800, function(err, found) {
          if (err) { res.writeHead(502); res.end(JSON.stringify({ ok: false, error: err.message })); return; }
          if (!found) {
            res.writeHead(200); res.end(JSON.stringify({ ok: false, error: 'No response from IDs 1–9' }));
            return;
          }
            ...
            scanMdcId([1,2,3,4,5,6,7,8,9], 800, function(err, found) {
              ...
              if (!found) {
                ...
                res.end(JSON.stringify({
                  ...
                  rj45Sent: true,
                  idOk: false,
                  error: 'RJ45 SET broadcast sent but no device responded to IDs 1–9',
                }));
                return;
              }
              res.writeHead(200);
