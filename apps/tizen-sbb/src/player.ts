...
// Legacy SSSP B2B API global (Samsung older firmware, not in tizen-tv-webapis types)
...
const Player = {
  ...
  _liveCaptureTimer: undefined as number | undefined, // setInterval handle for live-view frame loop
  ...
  _zoneAVPlayerMap: {} as Record<string, any>,
  _zoneSyncEnabled: false as boolean,
  _zoneDocumentActive: false as boolean,
  ...
  connectWebSocket(): void {
    try {
      ...
      this.wsConnection.onopen = () => {
        ...
        // Refresh MDC poll timestamp shortly after connect, so the UI doesn't
        // show a stale "polled Xm ago" if WS was down when the interval fired.
        setTimeout(() => { this.runMdcPoll(); }, 5000);
      };
      ...
      switch (messageType) {
        ...
        case 'remote_status': {
          ...
          function sendMdcStatusResponse(payload: Record<string, unknown> & { ok: boolean }) {
            if (rsRequestId && rsWs && rsWs.readyState === (rsWs as any).OPEN) {
          ...
          rsXhr.onerror   = function() { sendMdcStatusResponse({ ok: false, nodeRunning: true, error: 'MDC bridge XHR error' }); };
          rsXhr.ontimeout = function() { sendMdcStatusResponse({ ok: false, nodeRunning: true, error: 'MDC bridge timeout' }); };
          rsXhr.send();
          break;
        }

        case 'remote_key': {
          const keyName: string = (message.payload?.key ?? '') as string;
          logger.info('remote_key received:', keyName);
          // Forward to local MDC bridge Node server (127.0.0.1:9615).
          // Use XHR — fetch() is not available on Tizen 4.0 WebKit.
        ...
        case 'mdc_control': {
          ...
          if (mdcPayload && typeof mdcPayload.action === 'string') {
            ...
            // ── b2bapis intercepts (browser-side only, not available in server.js) ──
            if (action === 'b2b_pc_connection_get') {
              try {
                const b2b = typeof b2bapis !== 'undefined' ? (b2bapis as any).b2bcontrol : null;
                if (!b2b || typeof b2b.getPCConnection !== 'function') {
                  sendMdcControlResponse({ ok: false, error: 'b2bapis.b2bcontrol.getPCConnection not available on this firmware' });
                  break;
                }
                const result = b2b.getPCConnection();
                sendMdcControlResponse({ ok: true, raw: result, value: result });
              } catch (err: any) {
                sendMdcControlResponse({ ok: false, error: String(err?.message ?? err) });
              }
              break;
            }
            ...
            xhr.onload = function() {
              try {
                const response = JSON.parse(xhr.responseText) as { ok?: boolean; rawHex?: string; data?: number[]; error?: string };
                ...
                sendMdcControlResponse({
                  ok: !!response.ok,
                  ...(response.rawHex !== undefined ? { rawHex: response.rawHex } : {}),
        ...
        case 'start_live_capture':
          logger.info('start_live_capture received:', message.payload);
  ...
  sendWebSocketHeartbeat() {
    ...
    // Phase 3: fire MDC status_get and send mdc_heartbeat (non-blocking)
  ...
  startTelemetry() {
    ...
    this.telemetryInterval = setInterval(async () => {
      ...
      // Phase 4: run full MDC poll after each telemetry cycle
      this.runMdcPoll();
  ...
  runStartupMdcSetup(): void {
    ...
    const self = this;
    // Set MDC Connection Type to RJ45
    self.sendLocalMdcXhr('mdc_conn_type_set', { value: 1 })
      .then((r) => { logger.info('[mdc-startup] conn type RJ45:', r.ok); })
      .catch(() => { /* non-blocking */ });
    // Scan MDC device ID — result stored in server.js DEVICE_MDC_ID in-memory
    self.sendLocalMdcXhr('mdc_id_scan')
      ...
      .catch(() => { /* non-blocking */ });
    // Enable Network Standby so MDC works while panel is in standby
  ...
  runPostPairingMdcSetup(): void {
    ...
    const self = this;
    // Persist scanned MDC ID to DB (ws.ts merges into devices.settings.mdcId)
    ...
    // Ensure Network Standby is ON so the device is wakeable after power-off.
    // Phase 1 sets it before pairing but has no WS; this re-confirms it and the
    // subsequent runMdcPoll (5s after WS connect) writes the confirmed value to DB.
    self.sendLocalMdcXhr('network_standby_set', { value: 1 })
      .then((r) => { logger.info('[mdc-startup] network standby ON (phase 2):', r.ok); })
      .catch(() => { /* non-blocking */ });
    // Set Standby Control to Off
    self.sendLocalMdcXhr('standby_set', { value: 0 })
      .then((r) => { logger.info('[mdc-startup] standby OFF:', r.ok); })
      .catch(() => { /* non-blocking */ });
    // Disable Source(0), No Signal(2), MDC(3), Schedule(4) OSD overlays
    [0, 2, 3, 4].forEach((osdType) => {
      self.sendLocalMdcXhr('osd_display_set', { osdType, osdOnOff: 0 })
        .then((r) => { logger.info('[mdc-startup] OSD type', osdType, 'off:', r.ok); })
        .catch(() => { /* non-blocking */ });
    });
  },
  ...
  sendMdcHeartbeat(): void {
    ...
    xhr.onload = function() {
      try {
        const r = JSON.parse(xhr.responseText) as { ok: boolean; status?: { power?: number; volume?: number; mute?: number; input?: number } };
        if (!r.ok || !r.status) return;
        if (ws.readyState !== WebSocket.OPEN) return;
        ws.send(JSON.stringify({
          type: 'mdc_heartbeat',
          payload: { power: r.status.power, volume: r.status.volume, mute: r.status.mute, input: r.status.input },
        }));
      } catch { /* non-blocking */ }
    };
    xhr.onerror   = function() { /* non-blocking */ };
    xhr.ontimeout = function() { /* non-blocking */ };
    xhr.send(JSON.stringify({ action: 'status_get' }));
  },
  ...
  runMdcPoll(): void {
    ...
    let pending = commands.length + TIMER_SLOTS.length;
    function done() {
      pending--;
      if (pending > 0) return;
      if (ws.readyState !== WebSocket.OPEN) return;
      ...
      // Build timer slot array (index 0 = slot 1)
    ...
    for (const action of commands) {
      ...
      (function(a: string) {
        xhr.onload = function() {
          try { results[a] = JSON.parse(xhr.responseText); } catch { results[a] = { ok: false }; }
          done();
        };
        xhr.onerror   = function() { results[a] = { ok: false }; done(); };
        xhr.ontimeout = function() { results[a] = { ok: false }; done(); };
        xhr.send(JSON.stringify({ action: a }));
      })(action);
    }
    // Fetch all 7 on/off timer slots in parallel
    for (const slotNum of TIMER_SLOTS) {
      const xhr = new XMLHttpRequest();
      xhr.open('POST', 'http://127.0.0.1:9615/mdc-control', true);
      xhr.setRequestHeader('Content-Type', 'application/json');
      xhr.timeout = 8000;
      (function(s: number) {
        const key = `timer_${s}`;
        xhr.onload = function() {
          try { results[key] = JSON.parse(xhr.responseText); } catch { results[key] = { ok: false }; }
          done();
        };
        xhr.onerror   = function() { results[key] = { ok: false }; done(); };
        xhr.ontimeout = function() { results[key] = { ok: false }; done(); };
        xhr.send(JSON.stringify({ action: 'on_timer_get', slot: s }));
      })(slotNum);
    }
  },
  ...
  executeCommand(command) {
    ...
    switch (type) {
      ...
      case 'START_LIVE_CAPTURE': {
        const ms = Math.max(1000, (payload && (payload as any).intervalMs) || 1000);
      ...
      if (b2b && typeof b2b.captureScreen === 'function') {
        b2b.captureScreen(
          (filePath: string) => {
            ...
            try {
              (tizen as any).filesystem.resolve(filePath, (file: any) => {
                file.openStream('r', (stream: any) => {
                  try {
                    ...
                    send(btoa(binary));
                  } catch (e) { logger.warn('[Screenshot] read stream bytes failed:', e); }
                }, (e: any) => logger.warn('[Screenshot] openStream error:', e), 'ISO-8859-1');
              }, (e: any) => logger.warn('[Screenshot] filesystem.resolve failed:', e), 'r');
  ...
  takeScreenshotLive(): void {
      ...
      if (b2b && typeof b2b.captureScreen === 'function') {
        b2b.captureScreen(
          (filePath: string) => {
              ...
              (tizen as any).filesystem.resolve(filePath, (file: any) => {
                file.openStream('r', (stream: any) => {
                  try {
                    ...
                    for (let i = 0; i < bytes.length; i++) binary += String.fromCharCode(bytes[i]);
                    send(btoa(binary));
                  } catch (e) { logger.warn('[LiveCapture] read stream bytes failed:', e); done(); }
            ...
            } catch (e) { logger.warn('[LiveCapture] filesystem access failed:', e); done(); }
          },
          (e: any) => { logger.warn('[LiveCapture] captureScreen error:', (e && e.message) || e); done(); },
        );
        return;
      }
    } catch (e) { logger.warn('[LiveCapture] b2b captureScreen threw:', e); }
    // Canvas fallback
    try {
      const canvas = document.createElement('canvas');
      canvas.width = window.innerWidth || 1920;
    ...
    } catch (e) { logger.warn('[LiveCapture] Canvas fallback failed:', e); }
    done();
  },
  ...
  _applyHardwareNtp(server: string, timezone: string) {
    const b2b: any = typeof b2bapis !== 'undefined' ? (b2bapis as any).b2bcontrol || null : null;
    if (!b2b) {
      logger.debug('[NTP] b2bapis.b2bcontrol not available; hardware NTP skipped');
      return;
    }
    const onOk = (method: string) => () => logger.info('[NTP] hardware NTP set via', method);
    const onErr = (method: string) => (error: any) => logger.warn('[NTP] hardware NTP error via', method, (error && error.message) || error);
    if (typeof b2b.setntpuse === 'function') {
      try { b2b.setntpuse('use', onOk('setntpuse'), onErr('setntpuse')); }
      catch (error) { logger.warn('[NTP] setntpuse threw:', (error && error.message) || error); }
    }
    if (typeof b2b.setntpserveraddress === 'function') {
      try { b2b.setntpserveraddress(server, onOk('setntpserveraddress'), onErr('setntpserveraddress')); }
      catch (error) { logger.warn('[NTP] setntpserveraddress threw:', (error && error.message) || error); }
    }
    if (typeof b2b.setcurrenttimezone === 'function') {
      try { b2b.setcurrenttimezone(timezone, onOk('setcurrenttimezone'), onErr('setcurrenttimezone')); return; }
      catch (error) { logger.warn('[NTP] setcurrenttimezone threw:', (error && error.message) || error); }
    }
    if (typeof b2b.setNTPSync === 'function') {
      try { b2b.setNTPSync(server, timezone, onOk('setNTPSync'), onErr('setNTPSync')); return; }
      catch (error) { logger.warn('[NTP] setNTPSync threw:', (error && error.message) || error); }
    }
    if (typeof b2b.setClockSync === 'function') {
      try { b2b.setClockSync(true, server, timezone, onOk('setClockSync'), onErr('setClockSync')); return; }
      catch (error) { logger.warn('[NTP] setClockSync threw:', (error && error.message) || error); }
    }
    if (typeof b2b.setNTPServer === 'function') {
      try { b2b.setNTPServer(server, onOk('setNTPServer'), onErr('setNTPServer')); }
      catch (error) { logger.warn('[NTP] setNTPServer threw:', (error && error.message) || error); }
    } else if (typeof b2b.enableNTPSync === 'function') {
      try { b2b.enableNTPSync(server, onOk('enableNTPSync'), onErr('enableNTPSync')); }
      catch (error) { logger.warn('[NTP] enableNTPSync threw:', (error && error.message) || error); }
    }
    if (typeof b2b.setTimeZone === 'function') {
      try { b2b.setTimeZone(timezone, onOk('setTimeZone'), onErr('setTimeZone')); }
      catch (error) { logger.warn('[NTP] setTimeZone threw:', (error && error.message) || error); }
  ...
  stopZoneMode(): void {
    ...
    this._videoMixerQueue = Promise.resolve();
    ...
    if (this._zoneDocumentActive) {
      ...
      try { (window as any).b2bapis?.b2bdoc?.stopDoc(() => {}, () => {}); } catch (_) {}
      try { (window as any).b2bapis?.b2bcontrol?.closeDoc(() => {}, () => {}); } catch (_) {}
    }
  ...
  // Render Office/PPT/DOC document in a zone using b2bapis.b2bdoc (SSSP Document API).
  ...
  _playZoneOffice(zone: any, container: HTMLElement, content: any, items: any[], itemIndex: number, durationMs: number, token: string, zoneIndex: number): void {
    ...
    const docApi = (window as any).b2bapis?.b2bdoc;
    const controlApi = (window as any).b2bapis?.b2bcontrol;
    if (!docApi) {
      logger.warn(`[Zone ${zoneIndex}] b2bapis.b2bdoc unavailable — skipping OFFICE item`);
      const t = setTimeout(() => this._playZoneItems(zone, container, items, itemIndex + 1, token, zoneIndex), durationMs) as unknown as number;
      this._zoneTimers.push(t);
      return;
    }

    // Only one document instance system-wide
    if (this._zoneDocumentActive) {
      logger.warn(`[Zone ${zoneIndex}] b2bdoc already in use by another zone, skipping`);
      const t = setTimeout(() => this._playZoneItems(zone, container, items, itemIndex + 1, token, zoneIndex), durationMs) as unknown as number;
      this._zoneTimers.push(t);
      return;
    }
    ...
    const advanceOnce = () => {
      ...
      if (this._zoneDocumentActive) {
        this._zoneDocumentActive = false;
        try { docApi.stopDoc(() => {}, () => {}); } catch (_) {}
        try { controlApi?.closeDoc(() => {}, () => {}); } catch (_) {}
      }
      if (this._zoneMode && container.parentNode) {
        this._playZoneItems(zone, container, items, itemIndex + 1, token, zoneIndex);
      }
    ...
    try {
      // SSSP openDoc takes individual args: url, rectX, rectY, rectWidth, rectHeight, onSuccess, onError
      docApi.openDoc(url, zone.rect.x, zone.rect.y, zone.rect.width, zone.rect.height, () => {
        logger.info(`[Zone ${zoneIndex}] Document opened, playing at ${pageIntervalSecs}s/page`);
        try {
          docApi.playDoc(pageIntervalSecs, () => {
            logger.info(`[Zone ${zoneIndex}] Document playing`);
          }, (err: any) => {
            logger.warn(`[Zone ${zoneIndex}] Document play error:`, err);
          });
        } catch (e) {
          logger.warn(`[Zone ${zoneIndex}] docApi.playDoc threw:`, e);
        }
      }, (err: any) => {
      ...
      logger.warn(`[Zone ${zoneIndex}] docApi.openDoc threw:`, e);
      this._zoneDocumentActive = false;
    }
  ...
  // Detect which backend is available and store it for use in other sync methods.
  syncplayBackend: null as 'b2bapis' | 'webapis' | null,

  isSyncplayAvailable() {
    // Try b2bapis.b2bsyncplay first (Tizen 4 SBB / SSSP4)
    if (typeof b2bapis !== 'undefined' &&
        !!(b2bapis as any).b2bsyncplay &&
        typeof (b2bapis as any).b2bsyncplay.startSyncPlay === 'function' &&
        typeof (b2bapis as any).b2bsyncplay.makeSyncPlayList === 'function' &&
        typeof (b2bapis as any).b2bsyncplay.stopSyncPlay === 'function') {
  ...
  async prepareSyncPlaylistNative(data: any): Promise<boolean> {
    ...
    try {
      ...
      if (this.syncplayBackend === 'b2bapis') {
        try {
          (b2bapis as any).b2bsyncplay.clearSyncPlayList(
            (res: SyncplayResult) => logger.debug('Syncplay: pre-clean clearSyncPlayList ok', res?.result),
            (err: SyncplayError) => logger.debug('Syncplay: pre-clean clearSyncPlayList error (ignored)', err?.message)
          );
        } catch (e) {
      ...
      await new Promise((resolve, reject) => {
        const onSuccess = (res: SyncplayResult) => {
          logger.info('Syncplay: playlist created', res?.result, res?.data);
          resolve(true);
        };
        const onError = (err: SyncplayError) => {
          logger.error('Syncplay: createPlaylist failed', err?.name, err?.message, '(code:', err?.code, ')');
          reject(err);
        };

        if (this.syncplayBackend === 'b2bapis') {
          (b2bapis as any).b2bsyncplay.makeSyncPlayList(contentsArr, onSuccess, onError);
        } else {
    ...
    try {
      ...
      if (this.syncPlayListener) {
        try {
          ...
          if (this.syncplayBackend === 'b2bapis') {
            (b2bapis as any).b2bsyncplay.stopSyncPlay(this.syncPlayListener);
          } else {
            (webapis as any).syncplay.stop(this.syncPlayListener);
          }
        } catch (stopErr) {
      ...
      // b2bapis: startSyncPlay(x, y, w, h, groupID, rotate, onChange)
      // webapis: start(syncinfo, listener)
      const invokeStart = (syncinfo: SyncInfo) => {
        if (this.syncplayBackend === 'b2bapis') {
          (b2bapis as any).b2bsyncplay.startSyncPlay(
            syncinfo.rectX, syncinfo.rectY, syncinfo.rectWidth, syncinfo.rectHeight,
            syncinfo.groupID, syncinfo.rotate, listener
          );
        } else {
          (webapis as any).syncplay.start(syncinfo, listener);
        }
      };
      ...
      if (canProbeUhd) {
        ...
        try {
          ...
          try {
            if (this.syncplayBackend === 'b2bapis') {
              try { (b2bapis as any).b2bsyncplay.stopSyncPlay(listener); } catch (_) {}
            } else {
      ...
      try {
        if (this.syncplayBackend === 'b2bapis') {
          try { (b2bapis as any).b2bsyncplay.stopSyncPlay(this.syncPlayListener || (() => {})); } catch (_) {}
        } else {
  ...
  stopSyncPlayNative() {
    ...
    try {
      ...
      if (this.syncplayBackend === 'b2bapis') {
        (b2bapis as any).b2bsyncplay.stopSyncPlay(listener);
      } else {
    ...
    try {
      if (this.syncplayBackend === 'b2bapis') {
        (b2bapis as any).b2bsyncplay.clearSyncPlayList(
          (res: SyncplayResult) => logger.debug('Syncplay: clearSyncPlayList ok', res?.result),
          (err: SyncplayError) => logger.debug('Syncplay: clearSyncPlayList error (ignored)', err?.message)
        );
      } else {
        (webapis as any).syncplay.removePlaylist(
