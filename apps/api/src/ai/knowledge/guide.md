...
| Rule set | Automation rule: conditions + action + targets; fires when conditions are met | Workspace sidebar -> Rule Sets |
...
| "Automate content based on time or sensor" | Rule Sets | Create rule set -> add conditions -> choose action -> assign targets -> save |
| "Show an emergency message" | Rule Sets (Emergency Override action) or Emergency Override button | Rule Sets -> Emergency Override action, or use the siren icon in the top bar |
