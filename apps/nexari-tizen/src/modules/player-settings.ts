...
namespace PlayerSettings {
  ...
  function ensureOverlay(): void {
    ...
    _overlay.addEventListener('click', function(e) {
      if ((e as MouseEvent).target === _overlay) hide();
    });
  ...
  function startUpdate(): void {
    if (!_available || _updateInProgress || !_opts) return;
    ...
    const tiz: any = (window as any).tizen;
    let pkgId = 'com.nexari.player';
    try { pkgId = tiz.application.getCurrentApplication().appInfo.packageId || pkgId; } catch (_) {}
    ...
    AppUpdater.handle(
      { type: 'APP_UPDATE', wgtUrl: rel.downloadUrl, version: rel.version, packageId: pkgId },
      function(statusType: string, data?: Record<string, unknown>) {
        const pct = data && typeof data['pct'] === 'number' ? data['pct'] as number : null;
        if (bar && pct !== null) bar.style.width = pct + '%';
        if (statusType === 'app_update_downloading') {
          setMsg('Downloading… ' + (pct !== null ? pct + '%' : ''));
        } else if (statusType === 'app_update_installing') {
          const iPct = data && typeof data['installPct'] === 'number' ? data['installPct'] as number : null;
          if (bar && iPct !== null) bar.style.width = iPct + '%';
  ...
  export function setWsStatus(_connected: boolean): void {
