...
(function (WallSync) {
    ...
    let _followerReady = new Set();
    let _goSent = false;
    let _loadReceived = false;
    let _phaseTimer = null;
    let _peerHeads = new Map();
    let _peerWatchTimer = null;
    let _resyncInProgress = false;
    let _currentUrl = null; // most recent VIDEOWALL content URL
    // ── Public ─────────────────────────────────────────────────────────────────
    /**
    ...
    function init(cfg) {
        ...
        _loadReceived = false;
        _currentUrl = null;
        logger.info(`[WallSync] init deviceId=${cfg.deviceId} group=${cfg.groupId} ` +
        ...
        _bootAsync().catch((e) => {
            var _a;
            logger.error('[WallSync] boot failed: ' + ((_a = e === null || e === void 0 ? void 0 : e.message) !== null && _a !== void 0 ? _a : String(e)));
        });
    }
    WallSync.init = init;
    function stop() {
    ...
    function handleNewContent(url) {
        ...
        if (_role === 'leader' && _running && !_goSent && !_resyncInProgress) {
            ...
            _runLeader().catch((e) => {
                var _a;
                logger.error('[WallSync] _runLeader failed: ' + ((_a = e === null || e === void 0 ? void 0 : e.message) !== null && _a !== void 0 ? _a : String(e)));
            });
        }
    ...
    WallSync.handleNewContent = handleNewContent;
    // ── Boot sequence ──────────────────────────────────────────────────────────
    function _bootAsync() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            setTimeout(() => { if (!_stopped)
                _measureClock().catch(() => { }); }, 3000);
            setTimeout(() => { if (!_stopped)
                _measureClock().catch(() => { }); }, 10000);
            setInterval(() => { if (!_stopped)
                _measureClock().catch(() => { }); }, CLOCK_RESYNC_MS);
            setInterval(() => {
            ...
            const attempt = () => {
                ...
                try {
                    ...
                    ws.onclose = () => {
                        ...
                        if (!_stopped)
                            setTimeout(attempt, WS_RECONNECT_MS);
                    };
                }
                catch (e) {
                    logger.error(`[WallSync] WS open failed: ${e === null || e === void 0 ? void 0 : e.message}`);
                    if (!_stopped)
                        setTimeout(attempt, WS_RECONNECT_MS);
                }
            };
    ...
    function _wsSend(msg) {
        if (!_ws || !_wsReady || _ws.readyState !== 1 /* OPEN */)
            return;
        try {
            ...
            for (let i = 0; i < CLOCK_SAMPLES; i++) {
                setTimeout(() => {
                    ...
                    const onMsg = (ev) => {
                        ...
                        if (!msg || msg.type !== 'PONG' || msg.t1 !== t1)
                            return;
    ...
    const _localToServer = (t) => t + _offsetMs;
    const _serverToLocal = (t) => t - _offsetMs;
    // ── Peer discovery ─────────────────────────────────────────────────────────
    function _waitPeers() {
        return new Promise((resolve) => {
    ...
    function _dispatch(msg) {
        ...
        if (msg.type === 'PEERS' || msg.type === 'HEARTBEAT_PEERS') {
            const list = msg.type === 'PEERS'
                ? msg.peers.map((p) => p.deviceId)
                : msg.peers;
        ...
        if (msg.type === 'LOAD_URL') {
            if (_role !== 'follower')
                return;
            if (_loadReceived) {
                logger.info('[WallSync] LOAD_URL dup — ignored');
                return;
            }
            _loadReceived = true;
            _currentUrl = msg.url;
            ...
            if (typeof WallEngine !== 'undefined') {
                WallEngine.prepare(msg.url)
                    ...
                    .catch((e) => {
                    logger.error(`[WallSync] follower prepare failed: ${e === null || e === void 0 ? void 0 : e.message} — retry in 3s`);
                    if (!_stopped)
                        setTimeout(() => { _loadReceived = false; }, 3000);
                });
            }
            return;
        }
        ...
        if (msg.type === 'LOOP_GO') {
            ...
            if (typeof WallEngine !== 'undefined')
                WallEngine.schedulePlayAt(localPlayAt);
            return;
        }
    }
    // ── Leader flow ────────────────────────────────────────────────────────────
    function _runLeader() {
        return __awaiter(this, void 0, void 0, function* () {
            const url = _currentUrl !== null && _currentUrl !== void 0 ? _currentUrl : _cfg.getContentUrl();
            if (!url) {
            ...
            if (typeof WallEngine !== 'undefined') {
                WallEngine.prepare(url)
                    ...
                    .catch((e) => {
                    logger.error(`[WallSync] leader prepare failed: ${e === null || e === void 0 ? void 0 : e.message} — retry in 5s`);
                    if (!_stopped)
                        setTimeout(() => { if (!_stopped)
                            _runLeader().catch(() => { }); }, 5000);
                });
            }
        });
    }
    function _checkAllReady() {
        if (!_leaderReady || _followerReady.size < _peers.length || _goSent || _stopped)
            return;
        _goSent = true;
    ...
    function _peerScan() {
        if (_stopped || _resyncInProgress || _role !== 'leader')
            return;
    ...
    function _phaseTick() {
        if (_stopped)
            return;
        if (typeof WallEngine === 'undefined' || !WallEngine.isPlaying())
            return;
...
})(WallSync || (WallSync = {})); // namespace WallSync
