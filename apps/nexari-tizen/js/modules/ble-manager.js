// BLE Manager — Nexari Tizen Player
// Full Tizen Bluetooth LE implementation.
// Scans for beacons, evaluates proximity rules, and posts results to the server.
// Requires Tizen 5.5+ (getLEAdapter). On older firmware _checkSupport() returns
// false and all public methods become no-ops — no errors thrown.
window.BleManager = (function () {
   ...
   * getLEAdapter is only present on Tizen 5.5+ — on earlier firmware the
   * API simply doesn't exist, so the version guard in Platform is supplemented
   * by a runtime duck-type check here.
   */
  function _checkSupport() {
    // Fast path: honour Platform.supportsBle if set (avoids repeated tizen API probes)
    if (typeof Platform !== 'undefined' && Platform.supportsBle === false) {
      _supported = false;
      return false;
    }
    try {
  ...
  function _runScan(durationMs, onDone) {
    ...
    // Stop scan after duration and call onDone immediately — don't block on
  ...
  /**
   * Given the current beacon list, evaluate all rules (sorted by priority desc)
   ...
   * Supports: play_playlist, play_content.
   */
  function _applyRule(rule) {
    ...
    try {
      if (action.type === 'play_playlist' && action.playlistId) {
        if (typeof Player !== 'undefined' && Player.overridePlaylistForRule) {
          Player.overridePlaylistForRule(rule.id, action.playlistId, null);
        }
      } else if (action.type === 'play_content' && action.contentId) {
        if (typeof Player !== 'undefined' && Player.overridePlaylistForRule) {
          Player.overridePlaylistForRule(rule.id, null, action.contentId);
        }
      }
    } catch (e) {
  ...
  function _clearActiveRule() {
    if (_activeRuleId != null) {
      ...
      try {
        if (typeof Player !== 'undefined' && Player.clearRuleOverride) {
          Player.clearRuleOverride();
        }
      } catch (e) {}
    }
  }
  ...
  function _postScanResults(beaconList) {
    try {
      ...
      var token   = localStorage.getItem('deviceToken')
        || (typeof window.Player !== 'undefined' ? window.Player.deviceToken : null)
        || '';
  ...
  return {
     ...
     * No-op on Tizen < 5.5 where getLEAdapter is unavailable.
     */
    start: function () {
      if (!_checkSupport()) {
        _log('warn', 'Bluetooth LE not supported on this device (requires Tizen 5.5+)');
        return;
      }
      _startPeriodicScan();
    },
    ...
    setRules: function (rules) {
      ...
      _startPeriodicScan();
      // Evaluate immediately — don't wait up to 15s for the first periodic tick
      if (!_scanning) {
        _runScan(PERIODIC_SCAN_MS, function (beaconList) {
          _postScanResults(beaconList);
