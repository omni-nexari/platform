/**
 * MDC Bridge — local Node.js HTTP server running ON the Samsung LFD display (Tizen 6.5+).
 *
 * Started by app.js via:
 *   b2bapis.b2bcontrol.startNodeServer('../server.js', 'mdc-bridge', ...)
 *
 * The browser-side player calls:
 *   POST http://127.0.0.1:9615/mdc-control  { action: 'status_get' }
 *   POST http://127.0.0.1:9615/remote-key   { key: 'ARROW_UP' }
 *
 * This server translates MDC actions into Samsung MDC binary packets and
 * sends them to the TV firmware via TCP on localhost:1515 (MDC port).
