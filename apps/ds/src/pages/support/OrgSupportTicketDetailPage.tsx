...
export default function OrgSupportTicketDetailPage() {
  ...
  const { data: ticket, isLoading } = useQuery({
    queryKey: ['org-support-ticket', id],
    queryFn: () => api.get<SupportTicketDetail>(`/support/tickets/${id}`),
  ...
  const replyMut = useMutation({
    ...
    onSuccess: () => {
      setReply('');
      setPendingFiles([]);
      void qc.invalidateQueries({ queryKey: ['org-support-ticket', id] });
    ...
    if (pendingFiles.length > 0) {
      ...
      try {
        const uploads = await Promise.all(pendingFiles.map(async (file) => {
          ...
          fd.append('file', file);
          const res = await api.postForm<{ url: string }>(`/support/tickets/${id}/attachments`, fd);
          return res.url;
        }));
    ...
    <div className="page-container space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          <button onClick={() => navigate('/support')} className="text-[var(--text-muted)] hover:text-[var(--text)]">
            <ArrowLeft size={18} />
          </button>
      ...
      <SectionCard>
        ...
        <SectionCardBody className="space-y-6">
          {ticket.messages.length === 0 ? (
            <p className="text-sm text-[var(--text-muted)]">No messages yet. The support team will be in touch shortly.</p>
          ) : (
            ticket.messages.map(msg => (
              <MessageBubble key={msg.id} msg={msg} isMine={msg.senderType === 'client'} />
            ))
          )}
        </SectionCardBody>
      </SectionCard>
