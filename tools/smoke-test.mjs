...
const superadminEmail = process.env.SMOKE_SUPERADMIN_EMAIL ?? 'admin@admin.com';
const superadminPassword = process.env.SMOKE_SUPERADMIN_PASSWORD ?? 'q1w2e3r4';
const ownerEmail = process.env.SMOKE_OWNER_EMAIL ?? 'owner@acme.local';
const ownerPassword = process.env.SMOKE_OWNER_PASSWORD ?? 'Test@1234!';
const orgSlug = process.env.SMOKE_ORG_SLUG ?? 'acme';
const unique = process.env.SMOKE_RUN_ID ?? `smoke-${Date.now()}`;
const results = [];
const wsMessages = [];
let ws;
...
async function request(path, options = {}) {
  ...
  const res = await fetch(new URL(path, baseUrl), { ...options, headers });
...
async function main() {
  ...
  const org = orgs.data.find((item) => item.slug === orgSlug) || orgs.data[0];
  if (!org) {
  ...
  ws = new WebSocket(new URL(`/devices/ws/device?token=${encodeURIComponent(oldDeviceToken)}`, wsBaseUrl));
  ...
  console.log(JSON.stringify({ baseUrl: baseUrl.toString(), unique, results }, null, 2));
...
main().catch((error) => {
  ...
  console.error(JSON.stringify({ baseUrl: baseUrl.toString(), unique, results, fatal: String(error?.stack || error) }, null, 2));
  process.exit(1);
});